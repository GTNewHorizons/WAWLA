    ...
    public List<String> getWailaBody(Entity entity, List<String> tip, IWailaEntityAccessor data,
        ...
        if (entity instanceof EntityLiving && cfg.getConfig(CONFIG_EQUIPMENT)) {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                if (stack != null && data.getPlayer().isSneaking()) tip.add(
                        StatCollector.translateToLocal("tooltip.wawla." + itemTypes[i]) + ": "
        ...
        // shows pet owner
        if (cfg.getConfig(CONFIG_PET_OWNER) && data.getNBTData().hasKey("OwnerUUID")
                && data.getNBTData().getString("OwnerUUID").length() > 0)
            tip.add(
                    StatCollector.translateToLocal("tooltip.wawla.owner") + ": "
                            + Utilities.getUsernameByUUID(data.getNBTData().getString("OwnerUUID")));
        ...
        if (entity instanceof EntityTameable && cfg.getConfig(CONFIG_PET_SITTING)) {
            ...
            if (pet.isTamed()) tip.add(StatCollector.translateToLocal("tooltip.wawla.sit") + ": " + pet.isSitting());
        }
