...
public class AddonGenericTiles implements IWailaDataProvider {
    ...
    public List<String> getWailaBody(ItemStack stack, List<String> tip, IWailaDataAccessor data,
        ...
        if (tool != null && tool.equalsIgnoreCase("chisel")) {
            ...
            if (block == Blocks.stone) tool = "pickaxe";

            if (block == Blocks.planks) tool = "axe";
        }

        // Shows if the tool is the right tier.
        if (item != null
                && (item.getItem().getToolClasses(item).contains(tool) || AddonTinkersTiles.canHarvest(item, tool))) {

            // When the block is harvestable.
            if (cfg.getConfig(CONFIG_SHOW_HARVESTABILITY) && (blockLevel <= itemLevel || blockLevel == 0)) tip.add(
                    StatCollector.translateToLocal("tooltip.wawla.canHarvest") + ": "
                            + ((EnumChatFormatting.DARK_GREEN + StatCollector.translateToLocal("tooltip.wawla.yes"))));

            // When it's not harvestable.
            else {

                if (cfg.getConfig(CONFIG_SHOW_HARVESTABILITY)) tip.add(
                        StatCollector.translateToLocal("tooltip.wawla.canHarvest") + ": "
                                + EnumChatFormatting.RED
                                + StatCollector.translateToLocal("tooltip.wawla.no"));
                ...
                if (tool != null && cfg.getConfig(CONFIG_CORRECT_TOOL)) {
                    ...
                    else tip.add(StatCollector.translateToLocal("tooltip.wawla.toolType") + ": " + translation);
                }
            }
        }

        // Light level
        if (cfg.getConfig(CONFIG_LIGHTLEVEL) && (!data.getWorld().isBlockNormalCubeDefault(
                data.getPosition().blockX,
                data.getPosition().blockY + 1,
                data.getPosition().blockZ,
                false)
                || data.getWorld().isAirBlock(
                        data.getPosition().blockX,
                        data.getPosition().blockY + 1,
                        data.getPosition().blockZ))) {

            int dayLevel = Utilities.getBlockLightLevel(
                    data.getWorld(),
                    data.getPosition().blockX,
                    data.getPosition().blockY,
                    data.getPosition().blockZ,
                    true);
            int nightLevel = Utilities.getBlockLightLevel(
                    data.getWorld(),
                    data.getPosition().blockX,
                    data.getPosition().blockY,
                    data.getPosition().blockZ,
            ...
            if (cfg.getConfig(CONFIG_MONSTERLIGHT)) {
                ...
                if (nightLevel <= 7) display = display + EnumChatFormatting.DARK_RED + "" + nightLevel + " ";

                else if (nightLevel > 7) display = display + EnumChatFormatting.GREEN + "" + nightLevel + " ";
            }

            if (cfg.getConfig(CONFIG_DAYLIGHT)) display = display + EnumChatFormatting.YELLOW + "(" + dayLevel + ")";

            tip.add(display);
        }

        if (cfg.getConfig(CONFIG_HARDNESS)) tip.add(
                StatCollector.translateToLocal("tooltip.wawla.hardness") + ": "
                        + data.getBlock().getBlockHardness(
                                data.getWorld(),
                                data.getPosition().blockX,
                                data.getPosition().blockY,
                                data.getPosition().blockZ));

        if (cfg.getConfig(CONFIG_RESISTANCE)) tip.add(
                StatCollector.translateToLocal("tooltip.wawla.resistance") + ": "
                        + data.getBlock().getExplosionResistance(data.getPlayer()));
        ...
        // Beds
        if (cfg.getConfig(CONFIG_SLEEPY)
                && block.isBed(data.getWorld(), pos.blockX, pos.blockY, pos.blockZ, data.getPlayer()))
            tip.add(
                    StatCollector.translateToLocal("tooltip.wawla.sleepable") + ": "
                            + Utilities.canPlayerSleep(data.getPlayer()));
