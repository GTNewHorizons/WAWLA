...
public class AddonDeveloperTiles implements IWailaDataProvider {
    ...
    public List<String> getWailaBody(ItemStack stack, List<String> tip, IWailaDataAccessor data,
        ...
        boolean isKeySprinting = Minecraft.getMinecraft().gameSettings
                .isKeyDown(Minecraft.getMinecraft().gameSettings.keyBindSprint);

        if (data.getPlayer().isSneaking()) {

            tip.add("Block: " + data.getBlock().getClass().toString());

            if (data.getTileEntity() != null) tip.add("Class: " + data.getTileEntity().getClass().toString());
        }

        if (isKeySprinting) Utilities.wrapStringToList(data.getNBTData().toString(), 45, true, tip);
