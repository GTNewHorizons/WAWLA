...
public class AddonMorphEntities implements IWailaEntityProvider {
    ...
    public Entity getWailaOverride(IWailaEntityAccessor data, IWailaConfigHandler cfg) {
        ...
        if (data.getNBTData().hasKey(KEY_MORPH_ID)) morphID = data.getNBTData().getString(KEY_MORPH_ID);
    ...
    public NBTTagCompound getNBTData(EntityPlayerMP player, Entity entity, NBTTagCompound tag, World world) {
        ...
        if (entity != null && entity.getEntityData() != null) {
            ...
            NBTTagCompound morphTag = Utilities.getDeepTagCompound(
                    entity.getEntityData(),
                    new String[] { "PlayerPersisted", "MorphSave", "morphData", "nextState", "entInstanceTag" });

            if (morphTag.hasKey("id")) tag.setString(KEY_MORPH_ID, morphTag.getString("id"));
        }
