...
public class AddonJewelrycraftTiles implements IWailaDataProvider {
        ...
        if (data.getBlock() != null && data.getTileEntity() != null) {
            ...
            if (Utilities.compareTileEntityByClass(data.getTileEntity(), classTileEntitySmelter)) {
                ...
                if (data.getNBTData().getBoolean("hasMoltenMetal")) {
                    ...
                    if (cfg.getConfig(CONFIG_MOLTEN_AMOUNT)) tip.add(
                            StatCollector.translateToLocal("tooltip.wawla.jewelry.molten") + ": "
                                    + moltenStack.getDisplayName()
                                    + " X "
                                    + Utilities.round(data.getNBTData().getFloat("quantity") * 10, 2));
                }
                ...
                if (data.getNBTData().getBoolean("hasMetal")) {
                    ...
                    if (cfg.getConfig(CONFIG_METAL_AMOUNT)) tip.add(
                            StatCollector.translateToLocal("tooltip.wawla.jewelry.unmelted") + ": "
                                    + metalStack.getDisplayName()
                                    + " X "
                                    + metalStack.stackSize);

                    if (cfg.getConfig(CONFIG_MELTING_TIME)) tip.add(
                            StatCollector.translateToLocal("tooltip.wawla.jewelry.melttime") + ": "
                                    + data.getNBTData().getInteger("melting") / 20
                                    + " "
            ...
            if (Utilities.compareTileEntityByClass(data.getTileEntity(), classTileEntityMolder))
                if (cfg.getConfig(CONFIG_COOLING_TIME) && data.getNBTData().getInteger("cooling") > 0) tip.add(
                        StatCollector.translateToLocal("tooltip.wawla.jewelry.cooling") + ": "
                                + data.getNBTData().getInteger("cooling") / 20
                                + " "
                                + StatCollector.translateToLocal("tooltip.wawla.seconds"));
        }
    ...
    public ItemStack getMetalStack(NBTTagCompound metalTag) {
        ...
        return new ItemStack(
                Item.getItemById(metalTag.getShort("id")),
                metalTag.getByte("Count"),
                metalTag.getShort("Damage"));
    }
