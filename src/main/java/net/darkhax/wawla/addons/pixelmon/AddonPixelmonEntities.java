...
public class AddonPixelmonEntities implements IWailaEntityProvider {
    ...
    public List<String> getWailaBody(Entity entity, List<String> tip, IWailaEntityAccessor data,
        ...
        if (canDisplay && Utilities.compareByClass(classEntityPixelmon, entity.getClass())
                && data.getNBTData() != null) {

            NBTTagCompound tag = data.getNBTData();

            if (natureList != null && cfg.getConfig(CONFIG_NATURE)) tip.add(
                    StatCollector.translateToLocal("tooltip.wawla.pixelmon.nature") + ": "
                            + natureList[tag.getShort("Nature")]);

            if (cfg.getConfig(CONFIG_ABILITIES)) {

                EnumChatFormatting abilityColor = (tag.getInteger("AbilitySlot") == 2) ? EnumChatFormatting.GOLD
                        : EnumChatFormatting.GRAY;
                tip.add(
                        StatCollector.translateToLocal("tooltip.wawla.pixelmon.ability") + ": "
                                + abilityColor
                                + tag.getString("Ability"));
            }

            if (sizeList != null && cfg.getConfig(CONFIG_SIZE)) tip.add(
                    StatCollector.translateToLocal("tooltip.wawla.pixelmon.size") + ": "
                            + sizeList[tag.getShort("Growth")]);

            if (cfg.getConfig(CONFIG_FRIENDSHIP)) tip.add(
                    StatCollector.translateToLocal("tooltip.wawla.pixelmon.happiness") + ": "
                            + tag.getInteger("Friendship"));

            if (cfg.getConfig(CONFIG_HELD_ITEM) && tag.hasKey("HeldItemStack")) tip.add(
                    StatCollector.translateToLocal("tooltip.wawla.pixelmon.helditem") + ": "
                            + ItemStack.loadItemStackFromNBT((NBTTagCompound) tag.getTag("HeldItemStack"))
                                    .getDisplayName());

            if (cfg.getConfig(CONFIG_INDIVIDUAL_VALUES)) tip.add(
                    "IV's - Atk:" + tag.getInteger("IVAttack")
                            + " Def:"
                            + tag.getInteger("IVDefence")
                            + " HP:"
                            + tag.getInteger("IVHP")
                            + " SpAtk: "
                            + tag.getInteger("IVSpAtt")
                            + " SpDef:"
                            + tag.getInteger("IVSpDef")
                            + " Spd:"
                            + tag.getInteger("IVSpeed"));

            if (cfg.getConfig(CONFIG_EFFORT_VALUES) && !tag.getString("OwnerUUID").isEmpty()) tip.add(
                    "EV's - Atk:" + tag.getInteger("EVAttack")
                            + " Def:"
                            + tag.getInteger("EVDefence")
                            + " HP:"
                            + tag.getInteger("EVHP")
                            + " SpAtk: "
                            + tag.getInteger("EVSpAtt")
                            + " SpDef:"
                            + tag.getInteger("EVSpDef")
                            + " Spd:"
                            + tag.getInteger("EVSpeed"));
        }
