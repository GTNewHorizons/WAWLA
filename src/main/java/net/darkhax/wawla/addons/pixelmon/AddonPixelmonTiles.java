...
public class AddonPixelmonTiles implements IWailaDataProvider {
    ...
    public AddonPixelmonTiles() {
        ...
        if (Loader.isModLoaded("pixelmon")) {
            ...
            try {
                ...
                classTileEntityApricornTree = Class
                        .forName("com.pixelmonmod.pixelmon.blocks.apricornTrees.TileEntityApricornTree");
                classBlockApricornTree = Class
                        .forName("com.pixelmonmod.pixelmon.blocks.apricornTrees.BlockApricornTree");
            }
    ...
    public List<String> getWailaBody(ItemStack stack, List<String> tip, IWailaDataAccessor data,
        ...
        createApricornTooltip(
                data.getWorld().getTileEntity(
    ...
    /**
     * This is a custom method used to display information about an apricorn. The main reason of having this method is
     * due to the way apricorn blocks are handled. Apricorns are a double block structures and the top block does not
     * contain the correct information. This corrects for that.
     * 
    ...
    void createApricornTooltip(TileEntity entity, List<String> tip, Block block, IWailaConfigHandler cfg) {
        ...
        if (entity != null && Utilities.compareByClass(classTileEntityApricornTree, entity.getClass())) {
            ...
            if (cfg.getConfig(CONFIG_APRICORN_GROWTH)) tip.add(
                    StatCollector.translateToLocal("tooltip.wawla.pixelmon.growth") + ": "
                            + Utilities.round(Utilities.getProgression(meta, 5), 0)
                            + "%");

            if (cfg.getConfig(CONFIG_APRICORN_PRODUCT)) tip.add(
                    StatCollector.translateToLocal("tooltip.wawla.pixelmon.product") + ": "
                            + product.substring(9, product.length() - 5));
        }
    }
