...
public class AddonVanillaTiles implements IWailaDataProvider {
    ...
    public List<String> getWailaBody(ItemStack stack, List<String> tip, IWailaDataAccessor data,
        ...
        if (data.getTileEntity() instanceof TileEntityBeacon) {
            ...
            if (cfg.getConfig(CONFIG_BEACON_PRIMARY) && primary > 0) tip.add(
                    StatCollector.translateToLocal("tooltip.wawla.primary") + ": "
                            + StatCollector.translateToLocal(Potion.potionTypes[primary].getName()));

            if (cfg.getConfig(CONFIG_BEACON_SECONDARY) && secondary > 0) tip.add(
                    StatCollector.translateToLocal("tooltip.wawla.secondary") + ": "
        ...
        // Furnace
            ...
            if (burnTime > 0 && cfg.getConfig(CONFIG_FURNACE_BURNTIME)) tip.add(
                    StatCollector.translateToLocal("tooltip.wawla.burnTime") + ": "
                            + burnTime
                            + " "
            ...
            if (data.getPlayer().isSneaking()) {
                ...
                if (furnaceStacks[0] != null && cfg.getConfig(CONFIG_FURNACE_INPUT)) tip.add(
                        StatCollector.translateToLocal("tooltip.wawla.input") + ": "
                                + furnaceStacks[0].getDisplayName()
                                + " X "
                                + furnaceStacks[0].stackSize);

                if (furnaceStacks[1] != null && cfg.getConfig(CONFIG_FURNACE_FUEL)) tip.add(
                        StatCollector.translateToLocal("tooltip.wawla.fuel") + ": "
                                + furnaceStacks[1].getDisplayName()
                                + " X "
                                + furnaceStacks[1].stackSize);

                if (furnaceStacks[2] != null && cfg.getConfig(CONFIG_FURNACE_OUTPUT)) tip.add(
                        StatCollector.translateToLocal("tooltip.wawla.output") + ": "
                                + furnaceStacks[2].getDisplayName()
                                + " X "
                                + furnaceStacks[2].stackSize);
            }
        }

        // Player Skull
        if (data.getTileEntity() instanceof TileEntitySkull && cfg.getConfig(CONFIG_PLAYER_SKULL)
                && data.getNBTData().hasKey("Owner"))
            tip.add(
                    StatCollector.translateToLocal("tooltip.wawla.owner") + ": "
                            + NBTUtil.func_152459_a(data.getNBTData().getCompoundTag("Owner")).getName());
